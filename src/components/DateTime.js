...
export const DateTime = () => {
	...
	const normalTime = spacetime.now();
	//const [addBl, setAddBl] = useState();
	const addBl = document.querySelector('.additional__block');
		...
		<button className="btn" onClick={chageBtnTimeMode}>Transform clock to 12h version</button>
			<div className="promotion__timer">
				<div className="timer">
					...
					<div className="time additional__block hide"><p>{(normalTime.goto(selectedTimezone.value).hour() < 12) ? "AM" : "PM"}</p></div>
				</div>
			</div>
		</div>
		<div className="plus__minus" style={{flexWrap: "wrap", minWidth: "120px", margin: "20px"}}>
